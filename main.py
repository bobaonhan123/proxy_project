...
TARGETS = config.get('targets', ["127.0.0.1:3000"])  # List of target host:port pairs
BUFFER_SIZE = config.get('buffer_size', 4096)

# Global index to track the current target for round-robin
...
def handle_client(client_socket):
    try:
        ...
        if "Upgrade: websocket" in request_str:
            ...
            target_host, target_port = get_next_target()
